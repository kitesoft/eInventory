...
uses
  ...
  RzLabel,   MemDS, DBAccess, MyAccess, frxOLE, frxRich,
  frxCrypt, frxGZip, frxGradient, frxChBox, frxCross;
...
TString = class(TObject)
  ...
  private
    ...
    FCompany_Fax: string;
    FCompany_Tel: string;
    FCompany_Addr2: string;
    FCompany_Addr1: string;
  ...
  sql_fieldname, reportid: string; _params: TClientDataSet): boolean;
    ...
    procedure SetAutoReport(const Value: boolean);
    procedure SetCompany_Addr1(const Value: string);
    procedure SetCompany_Addr2(const Value: string);
    procedure SetCompany_Fax(const Value: string);
    procedure SetCompany_Name(const Value: string);
    procedure SetCompany_Tel(const Value: string);
    procedure SetRptGroup(const Value: string);
  public
    { Public declarations }
    ...
    property Company_Name : string read FCompany_Name write SetCompany_Name;
    property Company_Addr1: string read FCompany_Addr1 write SetCompany_Addr1;
    property Company_Addr2: string read FCompany_Addr2 write SetCompany_Addr2;
    property Company_Tel : string read FCompany_Tel write SetCompany_Tel;
    property Company_Fax : string read FCompany_Fax write SetCompany_Fax;


    property defaultValue : TList read FdefaultValue write SetdefaultValue;
    property AutoReport : boolean read FAutoReport write SetAutoReport;
    property RptGroup : string read FRptGroup write SetRptGroup;
  end;
...
uses CommonLIB, CommonUtils, STDLIB;
...
begin
  cdsRec.Close;
  cdsRec.CommandText:='SELECT * FROM  AppReport  WHERE RPTGROUP='''+FRptGroup+''' and ID=41';
  cdsRec.Open;
...
begin
  ...
  begin
   ...
   // if FAutoReport then frmGenReport.Width:=112;
   // if FAutoReport then frmGenReport.Height:=27;
...
begin
  ...
  cdsRepTemp.Close;
  cdsRepTemp.CommandText:='select * from  AppReport  where RPTGROUP='''+FRptGroup+''' and REPORT_ID='''+report_id+'''';
...
//                    if trim(cdsParamsObjValueVALUE.AsString)='' then
                  ...
                  //            ShowMessage(IntToStr(CdsName.RecordCount)) ;
                              ...
                              if trim(cdsParamsObj.FieldByName('VALUE').AsString)='#YEAR#' then
                              begin
                               DecodeDate(Now,yyyy,mm,dd);
                               for k:=yyyy downto yyyy-10 do
                               begin
                                  cmbItemCode := trim(inttostr(k)) ;
                                  cmbItemDesc := trim(inttostr(k+543)) ;
                              ...
                              if trim(cdsParamsObj.FieldByName('VALUE').AsString)='#MONTH#' then
                              begin

                                CdsName.Close ;
                                CdsName.CommandText := 'select MTTCDE as code,MTTDES as name from MTTCDE  where MTTGRP=''MONTH'' and MTTACT=''A'' order by CAST(MTTCDE as int)' ; //'select [key_id] as code, key_value as name from KVP_Table' ;
                                CdsName.Open ;
...
begin
        ...
        //_cdsTemp.ProviderName:='report_prov005';
         ...
         // _cdsTemp.RemoteServer:=scon;
          _cdsTemp.CommandText:='select * from  AppReport  where RPTGROUP='''+FRptGroup+''' and REPORT_ID='''+reportid+'''';
          _cdsTemp.Open;
...
procedure TfrmGenReport.SetAutoReport(const Value: boolean);
begin
  FAutoReport := Value;
end;
...
procedure TfrmGenReport.SetCompany_Addr1(const Value: string);
begin
  FCompany_Addr1 := Value;
end;

procedure TfrmGenReport.SetCompany_Addr2(const Value: string);
begin
  FCompany_Addr2 := Value;
end;

procedure TfrmGenReport.SetCompany_Fax(const Value: string);
begin
  FCompany_Fax := Value;
end;

procedure TfrmGenReport.SetCompany_Name(const Value: string);
begin
  FCompany_Name := Value;
end;

procedure TfrmGenReport.SetCompany_Tel(const Value: string);
begin
  FCompany_Tel := Value;
end;
...
//  TSQLConnection(cdsRepTemp.RemoteServer).AddProv('report_prov004', 'ALL');
 // cdsRepTemp.ProviderName:='report_prov004';
  //inputbox('','',FRptGroup);
  cdsRepTemp.CommandText:='select * from  AppReport  where RPTGROUP='''+FRptGroup+''' and REPORT_ID='''+reportid+'''';
       ...
       begin
         ...
         cdsReport5.close;
                  ...
                  begin
                      ...
                      frxReport.Variables['PARAM_LOCID'] :=QuotedStr(Branch);

                      // compay
                      frxReport.Variables['PARAM_CMPNAME'] :=QuotedStr(Company_Name);
                      frxReport.Variables['PARAM_CMPADDR1'] :=QuotedStr(Company_Addr1);
                      frxReport.Variables['PARAM_CMPADDR2'] :=QuotedStr(Company_Addr2);
                      frxReport.Variables['PARAM_CMPTEL'] :=QuotedStr(Company_Tel);
                      frxReport.Variables['PARAM_CMPFAX'] :=QuotedStr(Company_Fax);

                      //frxReport.Variables['PARAM_LOCNAME'] :=QuotedStr(getDonationsPointName(cdsTemp,Branch));
                      frxReport.PrepareReport();

                      if FAutoReport then
                      begin
                        if MessageDlg('[ Yes ] = Print '+#10#13+'[ Cancel ] = Preview',mtConfirmation,mbOKCancel,0)=mrCancel then
                        frxReport.ShowReport()
                        else frxReport.Print;
                      end else
                      frxReport.ShowReport();
                  end;
...
procedure TfrmGenReport.Timer1Timer(Sender: TObject);
begin
  if FAutoReport then Close;
end;
...
procedure TfrmGenReport.SetRptGroup(const Value: string);
begin
  FRptGroup := Value;
end;
