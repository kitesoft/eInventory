...
uses
  ...
  ,ImgList,Buttons,DBLookupEh,RzDBDTP;
...
const
 ...
 GridBgColor2=$00D2A4A4;
  _AppCompany=' (C) 2013,Siam Enterprise Software Dev Group';
 _AppName=' Inv2 ';
 _VersionCode='BETA'; //  Production Stable
 _VersionName='Standard';// Enterprise
 _VersionInternalNumber='0.56.17.11';

 _EnableSQLDebug = true;
...
function getServerDateOnly():TDate;
...
function loadCmbItems(CmbITem:TStrings;cds:TClientDataset;strSQL,fCode,fName,defaultCode:string):integer;overload;
function loadCmbItems(CmbITem:TStrings;cds:TClientDataset;fCode,fName,defaultCode:string):integer;overload;

procedure setSystemCMP(cds:TClientDataSet;_CompanyCode,BranchCode,DepartmentCode,SectionCode:string);
procedure setEntryUSRDT(cds:TClientDataSet;user:string);
procedure setModifyUSRDT(cds:TClientDataSet;user:string);

function getSqlCompanyCondition(als:string;_parameter:TDLLParameter):string;
...
begin
  ...
  if (strTmpStang<>'') then
    begin
      strTmpStang := strTmpStang + 'สตางค์';
  ...
  if (strTmpBaht<>'')  then
    begin
      ...
      strTmpBaht := StringReplace(strTmpBaht,'หนึ่งสิบ','สิบ',[rfReplaceAll]);
      strTmpBaht := StringReplace(strTmpBaht,'สิบหนึ่ง','สิบเอ็ด',[rfReplaceAll]);
      strTmpBaht := StringReplace(strTmpBaht,'สองสิบ','ยี่สิบ',[rfReplaceAll]);
...
function getSqlCompanyCondition(als:string;_parameter:TDLLParameter):string;
begin
  result := ' '+als+'CMPCOD='''+_parameter.Company+''' and '+als+'CMPBRN='''+_parameter.Branch+''' and '+als+'CMPDEP='''+_parameter.Department+''' and '+als+'CMPSEC='''+_parameter.Section+'''   ';
end;
...
function loadCmbItems(CmbITem:TStrings;cds:TClientDataset;fCode,fName,defaultCode:string):integer;
var i,defaultIndex: integer;
begin
  defaultIndex:=0;
  if cds.Active then
  ...
  begin
    ...
    _cds:=TClientDataSet.Create(nil);
      ...
      begin
        ...
        // MySQL
        _cds.data := GetDataSet('SELECT now() as curr_date');
  ...
  begin
    ...
    _cds:=TClientDataSet.Create(nil);
      ...
      begin
        ...
        // MySQL
        _cds.data := GetDataSet('SELECT year(now()) as curr_year');
        if recordcount>0 then
        begin
          yyyy:=FieldByName('curr_year').AsInteger;
        end;
        _cds.data := GetDataSet('SELECT month(now()) as curr_month');
        if recordcount>0 then
        begin
          mm:=FieldByName('curr_month').AsInteger;
        end;
        _cds.data := GetDataSet('SELECT day(now()) as curr_day');
        if recordcount>0 then
        begin
          dd:=FieldByName('curr_day').AsInteger;
        end;

        curr_date := EncodeDate(yyyy,mm,dd);
...
procedure setSystemCMP(cds:TClientDataSet;_CompanyCode,BranchCode,DepartmentCode,SectionCode:string);
begin
  cds.FieldByName('CMPCOD').AsString := _CompanyCode;
  cds.FieldByName('CMPBRN').AsString := BranchCode;
  cds.FieldByName('CMPDEP').AsString := DepartmentCode;
  cds.FieldByName('CMPSEC').AsString := SectionCode;
end;

procedure setEntryUSRDT(cds:TClientDataSet;user:string);
begin
  cds.FieldByName('ENTUSR').AsString := user;
  cds.FieldByName('ENTDTE').AsDateTime := getServerDate;
end;

procedure setModifyUSRDT(cds:TClientDataSet;user:string);
begin
  cds.FieldByName('EDTUSR').AsString := user;
  cds.FieldByName('EDTDTE').AsDateTime := getServerDate;
end;
...
function getBookBankID(_cds:TClientDataSet;objcode:string):String;
  ...
  begin
    ...
    begin
      ...
      if recordcount>0 then
      begin
        rep:=trim(FieldByName('BankBookID').AsString);
      end;
...
begin
  try
    ...
    if (xmlConn.ReadString('SmartLoans','ADDRESS','')='') then
    begin
        // mssql connection
        xmlConn.WriteString('SmartLoans','ADDRESS','127.0.0.1');
        xmlConn.WriteString('SmartLoans','HOSTNAME','dev.siamdev.org');
        xmlConn.WriteString('SmartLoans','USER','sa');
        xmlConn.WriteString('SmartLoans','PASSWORD','123456');
        xmlConn.WriteString('SmartLoans','DATABASE',_sample_db);
        xmlConn.Save;
    end;

     _app_address:= xmlConn.ReadString('SmartLoans','ADDRESS','');
     _app_hostname:= xmlConn.ReadString('SmartLoans','HOSTNAME','');
     _app_database:=xmlConn.ReadString('SmartLoans','DATABASE','');
     _app_user:=xmlConn.ReadString('SmartLoans','USER','sa');
     _app_password:=xmlConn.ReadString('SmartLoans','PASSWORD','123456');
        ...
        L := 0
           ...
           List.Insert(L, Parent.Controls[ctl]) ;
              ...
              begin
                 ...
                 //TRzEdit
                 if  Parent.Controls[ctl] is TRzButton then
                  begin
                     TRzButton(Parent.Controls[ctl]).TabStop:=false;
                  end;
                 ...
                 //TRzDBDateTimeEdit
                 if  Parent.Controls[ctl] is TRzDBDateTimePicker then
                  begin
                     TRzDBDateTimePicker(Parent.Controls[ctl]).FocusColor:=_focusColor;
                     TRzDBDateTimePicker(Parent.Controls[ctl]).FrameVisible:=_frameVisivle;
                  end;
                 ...
                 //TRzDBMemo
                 if  Parent.Controls[ctl] is TRzDBMemo then
                  begin
                     TRzDBMemo(Parent.Controls[ctl]).FocusColor:=_focusColor;
                     TRzDBMemo(Parent.Controls[ctl]).FrameVisible:=_frameVisivle;
                  end;
       ...
       begin
        ...
        L := 0
           ...
           List.Insert(L, Parent.Controls[ctl]) ;
             ...
             //TDBLookupComboboxEh
             if  Parent.Controls[ctl] is TDBLookupComboboxEh then
              begin
                 TDBLookupComboboxEh(Parent.Controls[ctl]).Enabled:=_enable;
                 if _enable then
                  TDBLookupComboboxEh(Parent.Controls[ctl]).Color:=_enableColor
                 else
                  TDBLookupComboboxEh(Parent.Controls[ctl]).Color:=_disableColor;
              end;
