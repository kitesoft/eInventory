...
begin
  ...
  if (FProdCode=0 ) or (FUnitCode=0) or (FPackingCode=0) or (FQty=0) or (FUnitPrice<=0)
  then
    begin
      Application.MessageBox(pchar('กรุณาระบุข้อมูลให้ถูกต้อง'),pchar('Warning'),MB_OK or MB_ICONWARNING);
      Exit;
    end;
...
//  _SQL :=' select UNICOD,UNINAM from ICMTTUNI  where UNIACT=''A'' order by UNICOD ';
  _SQL:= '  select b.PRUPRD,a.UNICOD,a.UNINAM  from ICMTTUNI a , ICMTTPRU b '+
   ' where a.UNIACT=''A'' and a.UNICOD = b.PRUUNI and b.PRUPRD = '+IntToStr(ProdCode)+
   ' group by b.PRUPRD,a.UNICOD,a.UNINAM order by a.UNICOD ';
...
begin
  ...
  edProdCode.Enabled :=FIsNew;
  edUnitCode.Enabled := FIsNew;
  edPackingCode.Enabled :=FIsNew;
  edProdCDE.Enabled := FIsNew;
end;
...
procedure TfrmAddProdItem.SetInvoiceCode(const Value: integer);
begin
  FInvoiceCode := Value;
end;
