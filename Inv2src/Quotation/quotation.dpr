...
uses
  ...
  uMasterQuotation in 'uMasterQuotation.pas' {frmMasterQuotation},
  DevCoreLIB in '..\LIB\DevCoreLIB.pas',
...
begin
    if _EnableSQLDebug then  Si.Enabled := True;

      Application := _MainApp;
      //Assign Screen Object
      //if Assigned(Scr) then Screen := Scr;
      //DLLForm2 := TDLLForm2.Create(Application);
      Result := TfrmMasterQuotation.Create(Application.MainForm);
      TfrmMasterQuotation(Result).DLLParameter := _Parameter;
