...
begin
  FPONetAmount := Value;
  lbNetAmount.Caption:=' รวมทั้งสิ้น  '+FormatCurr('#,##0.00',FPONetAmount)+' บาท';
end;
...
begin
  FPOVat := Value;
  lbVat.Caption:=' ภาษี  '+FormatCurr('#,##0.00',FPOVat)+' บาท';
end;
    ...
    if cdsReceiveDetails.RecordCount>0 then
      begin
        ...
        begin
          ...
          begin
            Application.MessageBox(pchar('กรุณาระบุวันหมดอายุก่อนทำการบันทึก!!!'),pchar('Warning'),MB_ICONWARNING or MB_OK);
            Exit;
          end;
          }
...
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              Application.MessageBox(pchar('ระบุจำนวนลงรับเกินจำนวนสั่งซื้อไม่สามารถ อนุมัติรายการได้!!!'),pchar('Warning'),MB_ICONWARNING or MB_OK);
              Exit;
            end;
  ...
  if Application.MessageBox(pchar('ยืนยันอนุมัติใบลงรับสินค้า !!!'),pchar('Confirm'),MB_OKCANCEL or MB_ICONQUESTION) = mrOk then
  begin
...
var
  ...
  reacode : integer;
begin
  reasonText := InputBox('ยกเลิกรายการ','กรุณาระบุเหตุผล การยกเลิก','');
  ...
  begin
    ...
    begin
      Application.MessageBox(pchar('ไม่สามารถยกเลิกรายการได้'),pchar('Warning'),MB_ICONWARNING or MB_OK);
      Exit;
    end;
...
begin
  ...
  begin
    ACanvas.DrawText(' อนุมัติ', RectText, cxAlignVCenter or cxAlignLeft or cxShowEndEllipsis);
    ACanvas.Brush.Color := $00DDFFBB;//clBlue;
  end
  else if  AViewInfo.GridRecord.Values[grdPOReceiveListFLG.Index] = 'N' then
  begin
    ACanvas.DrawText(' เอกสารใหม่', RectText, cxAlignVCenter or cxAlignLeft or cxShowEndEllipsis);
    ACanvas.Brush.Color := $00D2FFFF;;
  end
  else
    ACanvas.Brush.Color := clGray ;
